...
export enum DistanceMethod {
  // Fast straight-line distance calculation
  HAVERSINE = 'haversine',
  // Real-world walking path distance
  WALKING_PATH = 'walking-path',
  // Real-world driving path distance
  DRIVING_PATH = 'driving-path',
}

/**
 * Calculate distance between two points using the specified method
 */
export async function calculateDistance(
  ...
  switch (method) {
    ...
    case DistanceMethod.DRIVING_PATH:
      // Use Google API for driving path distance (not implemented yet)
      // For now, fall back to Haversine with a scaling factor
...
/**
 * Synchronous version that always uses Haversine
